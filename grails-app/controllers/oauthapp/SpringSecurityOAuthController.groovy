    ...
    def linkAccount = { OAuthLinkAccountCommand command ->
        ...
        if (request.post) {
            boolean linked = command.validate() && User.withTransaction { status ->
                //Don't compare the hash, won't match always. Use isPasswordValid.
                ...
                if (user) {
                    boolean pwdValid = passwordEncoder.isPasswordValid(user.password, command.password, null)
                    if(pwdValid) {
